...
class Iconic_WooThumbs_Core_Settings {
	...
	public static function add_settings_page() {
		if ( ! defined( 'ICONIC_DISABLE_BRAND' ) && ! defined( 'ICONIC_WOOTHUMBS_DISABLE_BRAND' ) ) {
			$default_title = sprintf( '<div style="height: 28px; line-height: 28px;"><img width="24" height="28" style="display: inline-block; vertical-align: middle; margin: 0 8px 0 0" src="%s"> %s by <a href="https://iconicwp.com/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=settings-title&utm_content=iconic-woothumbs" target="_blank">Iconic</a></div>', esc_attr( self::$iconic_svg ), self::$args['title'] );
		} else {
	...
	public static function hide_notices() {
		$hide_notice = filter_input( INPUT_GET, 'iconic-woothumbs-hide-notice', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
	...
	public static function output_getting_started_links() {
		...
		<ol>
			<?php foreach ( $links as $link ) { ?>
				<li>
					<a href="<?php echo esc_url( self::get_docs_url() . $link['href'] ); ?>?utm_source=Iconic&utm_medium=Plugin&utm_campaign=getting-started-links&utm_content=iconic-woothumbs" target="_blank"><?php echo esc_html( $link['title'] ); ?></a>
				</li>
			<?php } ?>
		</ol>
		<?php
	...
	public static function setup_dashboard( $settings ) {
		...
		$settings['tabs'][] = array(
			'id'    => 'dashboard',
			'title' => esc_html__( 'Dashboard', 'iconic-woothumbs' ),
		);
		...
		if ( current_user_can( 'manage_options' ) && ! defined( 'ICONIC_DISABLE_DASH' ) && ! defined( 'ICONIC_WOOTHUMBS_DISABLE_DASH' ) ) {
			$settings['sections']['license'] = array(
				'tab_id'              => 'dashboard',
				'section_id'          => 'general',
				'section_title'       => esc_html__( 'License &amp; Account Settings', 'iconic-woothumbs' ),
				'section_description' => '',
				'section_order'       => 10,
				'fields'              => array(
					array(
						'id'       => 'license_field',
						'title'    => __( 'License Key', 'iconic-woothumbs' ),
						'subtitle' => __( 'Activate your license to get the latest updates and support.', 'iconic-woothumbs' ),
						'type'     => 'custom',
						'output'   => '',
					),
					array(
			...
			$settings['sections']['support'] = array(
				'tab_id'              => 'dashboard',
				'section_id'          => 'support',
				'section_title'       => esc_html__( 'Support', 'iconic-woothumbs' ),
				'section_description' => '',
				'section_order'       => 30,
				'fields'              => array(
					array(
						'id'       => 'support',
						'title'    => esc_html__( 'Support', 'iconic-woothumbs' ),
						'subtitle' => esc_html__( 'Get premium support with a valid license.', 'iconic-woothumbs' ),
						'type'     => 'custom',
						'output'   => self::support_link(),
					),
					array(
						'id'       => 'documentation',
						'title'    => esc_html__( 'Documentation', 'iconic-woothumbs' ),
						'subtitle' => esc_html__( 'Read the plugin documentation.', 'iconic-woothumbs' ),
						'type'     => 'custom',
	...
	public static function add_sidebar() {
		...
		$api_sidebars = Iconic_WooThumbs_Core_Cross_Sells::get_api_sidebars();
		...
		<div class="iconic-settings-sidebar">
			...
			if ( ! empty( $cross_sells ) ) {
				...
				<div class="iconic-settings-sidebar__widget iconic-settings-sidebar__widget--works-well">
					<h3><?php esc_html_e( 'Works well with...', 'iconic-woothumbs' ); ?></h3>
					...
					<?php foreach ( $cross_sells as $cross_sell ) { ?>
						<div class="iconic-product">
							...
							<div class="iconic-product__content">
								<h4 class="iconic-product__title"><a target="_blank" href="<?php echo esc_url( $cross_sell['link'] ); ?>?utm_source=Iconic&utm_medium=Plugin&utm_campaign=cross-sell&utm_content=iconic-woothumbs" target="_blank"><?php echo wp_kses_post( $cross_sell['title']['rendered'] ); ?></a></h4>
									...
									<div class="iconic-product__buttons">
										<p>
											<a href="<?php echo esc_url( $checkout_url ); ?>" class="button iconic-button iconic-button--small" target="_blank">
												<?php esc_html_e( 'Buy Plugin', 'iconic-woothumbs' ); ?>
											</a>
										</p>
										<p><?php esc_html_e( '30 Day Money-Back Guarantee', 'iconic-woothumbs' ); ?></p>
									</div>
								<?php } ?>
							</div>
						</div>
	...
	public static function support_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://iconicwp.com/support/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=support-link&utm_content=iconic-woothumbs', esc_html__( 'Submit Ticket', 'iconic-woothumbs' ) );
	}
	...
	public static function documentation_link() {
		return sprintf( '<a href="%s?utm_source=Iconic&utm_medium=Plugin&utm_campaign=documentation-link&utm_content=iconic-woothumbs" class="button button-secondary" target="_blank">%s</a>', self::get_docs_url( 'collection' ), esc_html__( 'Read Documentation', 'iconic-woothumbs' ) );
	}
	...
	public static function account_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://my.iconicwp.com/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=account-link&utm_content=iconic-woothumbs', esc_html__( 'Manage Your Account', 'iconic-woothumbs' ) );
	}
	...
	public static function plugin_action_links( $links ) {
		...
		$action_links = array(
			'settings' => '<a href="' . admin_url( $settings_path ) . '" aria-label="' . esc_attr__( 'View settings', 'iconic-woothumbs' ) . '">' . esc_html__( 'Settings', 'iconic-woothumbs' ) . '</a>',
		);

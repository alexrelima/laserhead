...
use Iconic_WooThumbs_NS\StellarWP\Uplink\Resources\Plugin;
use Iconic_WooThumbs_NS\StellarWP\Uplink\Resources\Resource;
use Iconic_WooThumbs_NS\StellarWP\Uplink\Resources\Service;
use Iconic_WooThumbs_NS\StellarWP\Uplink\Uplink;
...
class License_Field extends Field {
	...
	public function register_settings(): void {
		foreach ( $this->get_resources() as $resource ) {
			...
			add_settings_field(
				$resource->get_license_object()->get_key_option_name(),
				__( 'License Key', 'iconic-woothumbs' ),
				[ $this, 'field_html' ],
				$this->get_group_name( sanitize_title( $resource->get_slug() ) ),
				self::get_section_name( $resource ),
				[
					...
					'placeholder'  => __( 'License Number', 'iconic-woothumbs' ),
	...
	public function get_field_html( $plugin ) : string {
		$html = sprintf( '<p class="tooltip description">%s</p>', __( 'A valid license key is required for support and updates', 'iconic-woothumbs' ) );

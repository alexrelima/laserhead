...
namespace Iconic_WooThumbs_NS\StellarWP\Uplink\Components\Admin;

use InvalidArgumentException;
use Iconic_WooThumbs_NS\StellarWP\Uplink\Auth\Admin\Disconnect_Controller;
use Iconic_WooThumbs_NS\StellarWP\Uplink\Auth\Auth_Url_Builder;
use Iconic_WooThumbs_NS\StellarWP\Uplink\Auth\Authorizer;
use Iconic_WooThumbs_NS\StellarWP\Uplink\Auth\Token\Contracts\Token_Manager;
use Iconic_WooThumbs_NS\StellarWP\Uplink\Components\Controller;
...
final class Authorize_Button_Controller extends Controller {
	...
	public function render( array $args = [] ): void {
		...
		if ( empty ( $slug ) ) {
			throw new InvalidArgumentException( __( 'The Product slug cannot be empty', 'iconic-woothumbs' ) );
		}
		...
		$authenticated = false;
		$target        = '_blank';
		$link_text     = __( 'Connect', 'iconic-woothumbs' );
		$classes       = [
			'uplink-authorize',
			'not-authorized',
		];
		...
		if ( ! $this->authorizer->can_auth() ) {
			$target    = '_self';
			$link_text = __( 'Contact your network administrator to connect', 'iconic-woothumbs' );
			$url       = get_admin_url( get_current_blog_id(), 'network/' );
		} elseif ( $this->token_manager->get() ) {
			$authenticated = true;
			$target        = '_self';
			$link_text     = __( 'Disconnect', 'iconic-woothumbs' );

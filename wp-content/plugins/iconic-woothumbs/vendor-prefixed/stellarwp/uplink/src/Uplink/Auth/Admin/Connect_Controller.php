...
namespace Iconic_WooThumbs_NS\StellarWP\Uplink\Auth\Admin;

use Iconic_WooThumbs_NS\StellarWP\Uplink\Auth\Nonce;
use Iconic_WooThumbs_NS\StellarWP\Uplink\Auth\Token\Connector;
use Iconic_WooThumbs_NS\StellarWP\Uplink\Auth\Token\Exceptions\InvalidTokenException;
use Iconic_WooThumbs_NS\StellarWP\Uplink\Notice\Notice_Handler;
use Iconic_WooThumbs_NS\StellarWP\Uplink\Notice\Notice;
	...
	public function maybe_store_token_data(): void {
		...
		if ( ! Nonce::verify( $args[ self::NONCE ] ?? '' ) ) {
			$this->notice->add( new Notice( Notice::ERROR,
				__( 'Unable to save token data: nonce verification failed.', 'iconic-woothumbs' ),
				true
			) );
		...
		try {
			if ( ! $this->connector->connect( $args[ self::TOKEN ] ?? '' ) ) {
				$this->notice->add( new Notice( Notice::ERROR,
					__( 'Error storing token.', 'iconic-woothumbs' ),
					true
				) );
		...
		if ( $license && $slug ) {
			if ( ! $this->collection->offsetExists( $slug ) ) {
				$this->notice->add( new Notice( Notice::ERROR,
					__( 'Plugin or Service slug not found.', 'iconic-woothumbs' ),
					true
				) );
			...
			if ( ! $plugin->set_license_key( $license, 'network' ) ) {
				$this->notice->add( new Notice( Notice::ERROR,
					__( 'Error storing license key.', 'iconic-woothumbs' ),
				true
				) );
		...
		$this->notice->add(
			new Notice( Notice::SUCCESS,
				__( 'Connected successfully.', 'iconic-woothumbs' ),
				true
			)
		);
	}

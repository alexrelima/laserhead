...
namespace Iconic_WooThumbs_NS\StellarWP\Uplink\API\V3\Auth;

use Iconic_WooThumbs_NS\StellarWP\Uplink\API\V3\Contracts\Client_V3;
use Iconic_WooThumbs_NS\StellarWP\Uplink\Traits\With_Debugging;
use WP_Error;
use WP_Http;
...
class Token_Authorizer implements Contracts\Token_Authorizer {
	...
	public function is_authorized( string $license, string $token, string $domain ): bool {
		...
		if ( $response instanceof WP_Error ) {
			if ( $this->is_wp_debug() ) {
				error_log( sprintf(
					__( 'Authorization error occurred: License: "%s", Token: "%s", Domain: "%s". Errors: %s', 'iconic-woothumbs' ),
					$license,
					$token,
					$domain,

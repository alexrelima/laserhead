...
namespace Iconic_WooThumbs_NS\StellarWP\Uplink\Notice;
	...
	public function __construct(
		...
		if ( ! in_array( $type, self::ALLOWED_TYPES, true ) ) {
			throw new InvalidArgumentException( sprintf(
					__( 'Notice $type must be one of: %s', 'iconic-woothumbs' ),
					implode( ', ', self::ALLOWED_TYPES ) )
			);
		}

		if ( empty( $message ) ) {
			throw new InvalidArgumentException( __( 'The $message cannot be empty', 'iconic-woothumbs' ) );
		}

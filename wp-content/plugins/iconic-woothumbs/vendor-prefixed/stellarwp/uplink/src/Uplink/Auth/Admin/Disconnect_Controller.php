...
use Iconic_WooThumbs_NS\StellarWP\Uplink\Auth\Token\Disconnector;
...
final class Disconnect_Controller {
	...
	public function maybe_disconnect(): void {
		...
		if ( wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), self::ARG ) ) {
			if ( $this->disconnect->disconnect() ) {
				$this->notice->add(
					new Notice( Notice::SUCCESS,
						__( 'Token disconnected.', 'iconic-woothumbs' ),
						true
					)
				);
			} else {
				$this->notice->add(
					new Notice( Notice::ERROR,
						__( 'Unable to disconnect token, ensure you have admin permissions.', 'iconic-woothumbs' ),
						true
					)
				);
			}
			...
			$this->notice->add(
				new Notice( Notice::ERROR,
					__( 'Unable to disconnect token: nonce verification failed.', 'iconic-woothumbs' ),
					true
				)
			);
		}
